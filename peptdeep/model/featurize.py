...
# %% auto 0
__all__ = ['mod_elements', 'MOD_TO_FEATURE', 'mod_feature_size', 'instrument_dict', 'unknown_inst_index', 'all_used_mod_elements',
           'encode_mod_formula', 'encode_mod_features', 'parse_mod_feature', 'get_batch_mod_feature',
...
def all_used_mod_elements(mod_formulas):
    """
...
def encode_mod_formula(formula, used_elements = mod_elements):
    '''
    Parse a modification formula to a feature vector
    '''
    el_idx = {el: i for i, el in enumerate(used_elements)}
    feature = np.zeros(len(used_elements))
    for el_cnt in formula.strip(')').split(')'):
        el, cnt = el_cnt.split('(')
        cnt = int(cnt)
        feature[el_idx[el]] = cnt
    return feature

def encode_mod_features(mod_formulas, used_elements=mod_elements):
    return {modname: encode_mod_formula(formula, used_elements)
            for modname, formula in mod_formulas.items()}

MOD_TO_FEATURE = encode_mod_features(MOD_CHEM)
